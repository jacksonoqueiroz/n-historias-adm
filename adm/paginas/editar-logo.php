...
if (empty($id_info)) {
  ...
  $query_info = "SELECT id, nome, logo FROM infograficos WHERE id=:id LIMIT 1";
...
<main role="main" class="container">
  ...
  if (!empty($dados['salvar'])) {
    ...
    if ((isset($arquivo['name'])) AND (!empty($arquivo['name']))) {
      ...
      $query_edit_img = "UPDATE infograficos SET logo=:logo, modifeld=NOW() WHERE id=:id";
      $edit_img = $conn->prepare($query_edit_img);
      $edit_img->bindParam(':logo', $arquivo['name'], PDO:: PARAM_STR);
      ...
      if ($edit_img->execute()) {
        ...
        if (move_uploaded_file($arquivo['tmp_name'], $diretorio . $nome_arquivo)) {
          ...
          if (((!empty($row_info['logo'])) or ($row_info['logo'] != null)) 
            AND ($row_info['logo'] != $arquivo['name'])) {
            // Exclui a imagem
            $endereco_imagem = "../assets/infograficos/$id_info/" . $row_info['logo'];
