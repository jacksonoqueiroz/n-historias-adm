...
if (empty($id_obra)) {
  ...
  $query_info = "SELECT obra.id_obra, 
                        obra.id_info, 
                        obra.img_obra,
                        info.id,
                        info.nome AS nome
                        ...
                        WHERE id_obra=:id_obra LIMIT 1";
  $result_info = $conn->prepare($query_info);
  $result_info->bindParam(':id_obra', $id_obra, PDO::PARAM_INT);
  $result_info->execute();
...
?>
<title>Obra Imagem | <?php echo $nome ?> </title>
...
<main role="main" class="container">
  <div class="jumbotron">
    <h1>Editar Imagem Obra | <?php echo $nome ?></h1>
 </div>
  ...
  if (!empty($dados['salvar'])) {
    ...
    if ((isset($arquivo['name'])) AND (!empty($arquivo['name']))) {
      ...
      $query_edit_img = "UPDATE obra SET img_obra=:img_obra, modifield=NOW() WHERE id_obra=:id_obra";
      $edit_img = $conn->prepare($query_edit_img);
      $edit_img->bindParam(':img_obra', $arquivo['name'], PDO:: PARAM_STR);
      $edit_img->bindParam(':id_obra', $id_obra, PDO:: PARAM_INT);
      ...
      if ($edit_img->execute()) {
        ...
        if (move_uploaded_file($arquivo['tmp_name'], $diretorio . $nome_arquivo)) {
          ...
          if (((!empty($row_info['img_obra'])) or ($row_info['img_obra'] != null)) 
            AND ($row_info['img_obra'] != $arquivo['name'])) {
            // Exclui a imagem
            $endereco_imagem = "../assets/infograficos/$id_info/" . $row_info['img_obra'];
          ...
          $_SESSION['msg_obra'] = "<div class='alert alert-success' id='msg-success' role='alert'>Imagem editado!</div>";
          header("Location: detalhes?id=$id_info");
        }else{
  ...
  ?>
    ...
    <form name="editar_capa" method="POST" action="" enctype="multipart/form-data">
      <div class="form-group">
        <label for="imagem">Imagem Obra:</label>
