...
if (empty($id_info)) {
  ...
  $query_info = "SELECT id, nome, thumb FROM infograficos WHERE id=:id LIMIT 1";
...
?>
<title>Logo | <?php echo $nome ?> </title>
...
<main role="main" class="container">
  <div class="jumbotron">
    <h1>Editar Logo | <?php echo $nome ?></h1>
 </div>
  ...
  if (!empty($dados['salvar'])) {
    ...
    if ((isset($arquivo['name'])) AND (!empty($arquivo['name']))) {
      ...
      $query_edit_img = "UPDATE infograficos SET thumb=:thumb, modifeld=NOW() WHERE id=:id";
      $edit_img = $conn->prepare($query_edit_img);
      $edit_img->bindParam(':thumb', $arquivo['name'], PDO:: PARAM_STR);
      ...
      if ($edit_img->execute()) {
        ...
        if (move_uploaded_file($arquivo['tmp_name'], $diretorio . $nome_arquivo)) {
          ...
          if (((!empty($row_info['thumb'])) or ($row_info['thumb'] != null)) 
            AND ($row_info['thumb'] != $arquivo['name'])) {
            // Exclui a imagem
            $endereco_imagem = "../assets/infograficos/$id_info/" . $row_info['thumb'];
  ...
  ?>
    ...
    <form name="editar_capa" method="POST" action="" enctype="multipart/form-data">
      <div class="form-group">
        <label for="imagem">Imagem Logo:</label>
